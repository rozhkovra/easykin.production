...
public class CalcBuilderFactory {
    public ICalcBuildBean water(int coldPrevMesure, int coldCurrentMesure, int coldPrevMesure2, int coldCurrentMesure2
    ...
    public ICalcBuildBean service(Date date, Collection<ICalculation> beans) {
        return new ServiceBean(date, beans);
    }

    public ICalcBuildBean measure(Collection<Integer> prevMeasure, Collection<Integer> currentMeasure, Money rate, boolean isPaid, CalculationType type) {
        return new MeasureBean(prevMeasure, currentMeasure, rate, isPaid, type);
    }

    public ICalcBuildBean defaultBean(CalculationType type, Money price, boolean isPaid) {
