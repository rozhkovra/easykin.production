...
import ru.rrozhkov.easykin.service.calc2.impl.convert.DBRateConverter;
...
public class RateHandler {
    ...
    public static Collection<IRate> select() throws Exception {
        return DBManager.instance().select(select, new DBRateConverter());
    }
    public static Collection<IRate> selectForDate(Date rateDate) throws Exception {
        return DBManager.instance().select(selectForDate.replace("#rateDate#", DateUtil.formatSql(rateDate)), new DBRateConverter());
    }
}
